...
def get_overworld_sprite_image_raw(rom: ROM, owsprite_address: int, sprite_subindex: int):
    with tempfile.TemporaryDirectory() as temporary_directory:
        ...
        if sprite_subindex >= sprite_count:
            raise ValueError(f"sprite index ({sprite_subindex}) out of range (sprite_count={sprite_count})")
        sprite_header_offsets = [
                struct.unpack_from("<I", data_with_header, 8 + sprite_subindex*4)[0]
                for sprite_subindex in range(sprite_count)
        ]
        data = data_with_header[8:]

        sprite_header_offset = sprite_header_offsets[sprite_subindex]
        ...
        for object_list_entry in object_list:
            ...
            for yi in range(0, object_height, TILE_HEIGHT):
                for xi in range(0, object_width, TILE_WIDTH):
                    if object_list_entry.palette_index >= len(tile_sets):
                        tile_set = tile_sets[0] # HACK
                    else:
                        tile_set = tile_sets[object_list_entry.palette_index]
